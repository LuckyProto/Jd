...
PicBox.prototype.init = function(){
	...
	$.each(this.picarr , function(){
		...
		var imgurl = this.imgUrl.replace("s128x96_jfs" , "s48x48_jfs");
		//这个局部的DOM形式：
		var _domstr = [
			'<li>',
...
PicBox.prototype.showPic = function(number){
	...
	//得到URL
	var url = this.picarr[number].imgUrl.replace("img30.360buyimg.com/n0/s128x96_jfs/","img30.360buyimg.com/shaidan/s616x405_jfs/");
	...
	if(number == this.picarr.length - 1){
		this.$rightBtn.css("cursor" , "url(images/small.cur),auto");
	}else{
		this.$rightBtn.css("cursor" , "url(images/next.cur),auto");
	}

	if(number == 0){
		this.$leftBtn.css("cursor" , "url(images/small.cur),auto");
	}else{
		this.$leftBtn.css("cursor" , "url(images/prev.cur),auto");
	}
}
//下一张
